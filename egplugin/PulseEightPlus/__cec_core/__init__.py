...
class PyCECAdapter(object):
    ...
    def __init__(self, cec_configuration):
        ...
        for device in self:
            if device.name == cec_configuration.connected_to:
                if not self._adapter.SetHDMIPort(device.logical_address, cec_configuration.iHDMIPort):
                    raise RuntimeError('Problem setting connection device ' + device.name)
    ...
    def end_last_event(self, _):
        pass

    # key press callback
    def __keypress_callback(self, key, duration):
        str_key = self._adapter.UserControlCodeToString(key).title()
        if duration == 0 and self._last_key_pressed != key:
            self._last_key_pressed = key
            self.trigger_enduring_event('RemoteKeyPress.{0}'.format(str_key))
        elif duration > 0 and self._last_key_pressed == key:
            self.end_last_event(duration)
            self._last_key_pressed = 255
        elif self._last_key_pressed != key:
            self._last_key_pressed = 255
            self.trigger_event('RemoteKeyPress.{0}'.format(str_key))
        return 0
