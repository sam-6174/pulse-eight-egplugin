...
from eg.UserPluginModule.PulseEightPlus import Text
...
class CheckListBox(wx.Panel):
    ...
    def SetToolTipString(self, index, tooltip):

        if index is None:
            wx.Panel.SetToolTipString(self, tooltip)
        else:
            self.texts[index].SetToolTipString(tooltip)
            self.ctrls[index].SetToolTipString(tooltip)
...
class AdapterListCtrl(FoldPanelBar):
    ...
    def add_adapter(self, adapter):
        ...
        first_colour = wx.SystemSettings_GetColour(wx.SYS_COLOUR_BTNFACE)
        second_colour = wx.SystemSettings_GetColour(wx.SYS_COLOUR_BTNSHADOW)
        ...
        def h_sizer(ctrl1, ctrl2, tooltip=None, prop=1):
            ...
            if not isinstance(ctrl1, wx.Sizer):
                ...
                if tooltip is not None:
                    ctrl1.SetToolTipString(tooltip)
        ...
        connected_to_st, connected_to_ctrl, connected_to_sizer = h_sizer(
            Text.connected_to_header,
            adapter[14],
            Text.connected_to_tooltip
        )
        ...
        adapter_line_3 = connected_to_sizer
        adapter_line_4 = type_sizer
        ...
        adapter_sizer.Add(adapter_line_4, 0, wx.EXPAND)
...
class AdapterConfig(object):
    ...
    def parent(self, fold_panel):
        ...
        self.__connected_to_ctrl = wx.Choice(parent, -1, choices=['TV'])
        self.__connected_to_ctrl.SetSelection(0)
        self.__connected_to_ctrl.SetMinSize((150, -1))
        ...
        self.__name_ctrl.SetToolTipString(Text.adapter_name_tooltip)
        self.__port_ctrl.SetToolTipString(Text.adapter_port_tooltip)
        self.__type_ctrl.SetToolTipString(None, Text.adapter_type_tooltip)
        self.__wake_avr_ctrl.SetToolTipString(Text.wake_avr_tooltip)
        self.__hdmi_ctrl.numCtrl.SetToolTipString(Text.hdmi_tooltip)
        self.__avr_audio_ctrl.SetToolTipString(Text.avr_audio_tooltip)
        self.__power_off_ctrl.SetToolTipString(Text.power_off_tooltip)
        self.__combo_ctrl.SetToolTipString(Text.keypress_combo_tooltip)
        self.__power_standby_ctrl.SetToolTipString(Text.power_standby_tooltip)
        self.__combo_timeout_ctrl.numCtrl.SetToolTipString(
            Text.keypress_combo_timeout_tooltip
        )
        self.__keypress_repeat_ctrl.numCtrl.SetToolTipString(
            Text.keypress_repeat_tooltip
        )
        self.__keypress_release_ctrl.numCtrl.SetToolTipString(
            Text.keypress_release_delay_tooltip
        )
        self.__keypress_double_ctrl.numCtrl.SetToolTipString(
            Text.keypress_double_tap_tooltip
        )

        self.__logging_ctrl.SetToolTipString(0, Text.info_tooltip)
        self.__logging_ctrl.SetToolTipString(1, Text.notice_tooltip)
        self.__logging_ctrl.SetToolTipString(2, Text.warning_tooltip)
        self.__logging_ctrl.SetToolTipString(3, Text.error_tooltip)
        self.__logging_ctrl.SetToolTipString(4, Text.debug_tooltip)
        self.__logging_ctrl.SetToolTipString(5, Text.log_file_tooltip)
    ...
    @property
    def connected_to_choices(self):
        return self.__connected_to_ctrl.GetItems()

    @connected_to_choices.setter
    def connected_to_choices(self, value):
        selection = self.__connected_to_ctrl.GetStringSelection()
        self.__connected_to_ctrl.Clear()
        self.__connected_to_ctrl.SetItems(value)
        if selection in value:
            self.__connected_to_ctrl.SetStringSelection(selection)
        else:
            self.__connected_to_ctrl.SetSelection(0)

    @property
    def connected_to(self):
        return self.__connected_to_ctrl.GetStringSelection()

    @connected_to.setter
    def connected_to(self, value):
        if value in self.__connected_to_ctrl.GetItems():
            self.__connected_to_ctrl.SetStringSelection(value)
        else:
            self.__connected_to_ctrl.SetSelection(0)
            ...
            if adapter is not None:
                ...
                for d in adapter:
                    if d.port is None:
                        ...
                        choices += [d.osd_name + '[HDMI ' + str(d.port) + ']']

                    if dev_name in (choices[-1], d.osd_name):
                        selection = choices[-1]
                ...
                if selection is None:
                    ...
                    device_ctrl.SetStringSelection(selection)

        def get_value():
            return device_ctrl.GetStringSelection().split('[')[0]
